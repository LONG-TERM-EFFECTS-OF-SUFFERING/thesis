...
\subsection{Existing open-source and academic tools}
...
While numerous open-source tools provide specific functionalities for YouTube data analysis, such as comment extraction libraries (e.g., YouTube Data API wrappers in Python like \texttt{youtube-search-python} \cite{youtube_search_python}), sentiment analysis packages (e.g., VADER with 4.5K GitHub stars, TextBlob with 9K stars, spaCy with 30K stars) and data visualization frameworks (e.g., Matplotlib, Plotly, D3.js), none combine data collection, processing, analysis and visualization into a unified, documented and methodologically transparent platform designed specifically for academic research purposes. Examples of fragmented tools include standalone projects like \texttt{youtube-comments-sentiment-analyzer} \cite{youtube_comments_sentiment} and \texttt{YouTube Data Tools} \cite{youtube_data_tools}, each addressing isolated components but requiring manual integration, custom scripting and technical expertise to create a complete research pipeline.
...
\subsection{Key academic studies}
...
\subsection{Interactions in the YouTube social network}
...
\subsection{YouTube Data API limitations and computational reproducibility challenges}

\label{subsec:youtube_data_api_limitations_and_computational reproducibility_challenges}
