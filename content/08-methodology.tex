...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{1.5cm}>{\raggedright\arraybackslash}p{4cm}X}
		...
		\midrule
		\label{act:2.2} A2.2 & Implement the LLM integration module that processes natural language user requests and generates valid YouTube API query parameters using an existing LLM model via API. & Source code for the NLP interface component that translates user queries into structured API parameters with validation mechanisms. \\
		\bottomrule
	\end{tabularx}
\end{table}
